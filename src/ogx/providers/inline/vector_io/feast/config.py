...
class FieldMapping(BaseModel):
    """Maps OGX's internal field names to the actual field names in an existing Feast feature view."""
...
class ExistingFeatureViewConfig(BaseModel):
    ...
    vector_store_id: str | None = Field(
        default=None,
        description="Vector store ID to expose in OGX. Defaults to feature_view_name.",
    )
    dimension: int = Field(
        description="Embedding dimension of the vectors in this feature view",
    )
    field_mapping: FieldMapping = Field(
        description="Maps OGX field names (embedding, chunk_id, chunk_text, chunk_metadata) to the actual field names in the existing Feast feature view",
    )
...
class FeastVectorIOConfig(BaseModel):
    ...
    existing_feature_views: list[ExistingFeatureViewConfig] | None = Field(
        ...
        description=(
            ...
            "Each entry maps an existing feature view's fields to OGX's expected schema. "
            "These feature views must already exist in the Feast registry."
        ),
    )
    persistence: KVStoreReference = Field(
        description="Config for KV store backend used by OGX for metadata persistence",
    )
