...
class FeastIndex(EmbeddingIndex):
    ...
    Each OGX VectorStore maps to a Feast FeatureView with a vector-indexed
    ...
    the Feast schema and OGX's internal field names. External feature views
    are never deleted by OGX.
    """
    ...
    def _parse_feast_response(self, response: Any, score_threshold: float) -> QueryChunksResponse:
        ...
        for _, row in result_df.iterrows():
            ...
            metadata = {}
            if self._chunk_metadata_field:
                raw_metadata = row.get(self._chunk_metadata_field)
                if raw_metadata is not None:
                    if isinstance(raw_metadata, dict):
                        ...
                        try:
                            ...
                            metadata = parsed if isinstance(parsed, dict) else {"value": raw_metadata}
